...
public sealed partial class BilibiliArticleParser(HttpClient http, PluginConfig config)
{
    ...
    private static readonly Regex HtmlBlockPattern = new("<(p|div|h[1-6]|blockquote|li|pre|code)([^>]*)>(.*?)</\\1>", RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex BrPattern = new("<br\\s*/?>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex BlockEndPattern = new("</(p|div|h[1-6]|li|blockquote)>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex TagPattern = new("<[^>]+>", RegexOptions.Compiled);
    private static readonly Regex MultiBlankLinePattern = new("\\n{3,}", RegexOptions.Compiled);

    private static Regex HtmlBlockRegex() => HtmlBlockPattern;

    private static Regex BrRegex() => BrPattern;

    private static Regex BlockEndRegex() => BlockEndPattern;

    private static Regex TagRegex() => TagPattern;

    private static Regex MultiBlankLineRegex() => MultiBlankLinePattern;
}
