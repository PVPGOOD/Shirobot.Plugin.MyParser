...
using MyParser.Provider.Xiaohongshu.Services;
using MyParser.Provider.Xiaohongshu.Utilities;
using ShiroBot.SDK.Abstractions;

namespace MyParser.Provider.Xiaohongshu.Parsing;

public sealed partial class XiaohongshuParser : IParserHttpClientAccessor, IDisposable
{
    ...
    private static readonly Regex InitialStatePattern = new("window\\.__INITIAL_STATE__\\s*=\\s*({.*?})\\s*</script>", RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex UndefinedPattern = new("\\bundefined\\b", RegexOptions.Compiled);
    private static readonly Regex MetaPattern = new("<meta\\b[^>]*>", RegexOptions.IgnoreCase | RegexOptions.Compiled);
    private static readonly Regex OriginVideoKeyPattern = new("\"originVideoKey\"\\s*:\\s*\"([^\"\\\\]*(?:\\\\.[^\"\\\\]*)*)\"", RegexOptions.Compiled);

    private static Regex InitialStateRegex() => InitialStatePattern;

    private static Regex UndefinedRegex() => UndefinedPattern;

    private static Regex MetaRegex() => MetaPattern;

    private static Regex OriginVideoKeyRegex() => OriginVideoKeyPattern;
...
public static class XiaohongshuJsonExtensions
{
