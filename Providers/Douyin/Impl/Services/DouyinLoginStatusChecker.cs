...
internal sealed class DouyinLoginStatusChecker(MyParserConfig config, HttpClient http)
{
    public async Task<string> CheckLoginStatusAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!config.DouyinCookie.Contains("sessionid=", StringComparison.OrdinalIgnoreCase)
            || !config.DouyinCookie.Contains("ttwid=", StringComparison.OrdinalIgnoreCase))
        {
        ...
        {
            return $"HTTP {(int)response.StatusCode}; cookie_length={config.DouyinCookie.Length}";
        }
        ...
        try
        {
            ...
            return statusCode == 0
                ? $"有效/已登录; status_code=0; has_data={hasUser}; cookie_length={config.DouyinCookie.Length}"
                : $"可能失效/未登录; status_code={statusCode}; message={message}; cookie_length={config.DouyinCookie.Length}";
        }
        catch (JsonException)
        {
            var sample = body.Length > 120 ? body[..120] : body;
            return $"返回非 JSON; cookie_length={config.DouyinCookie.Length}; sample={sample.ReplaceLineEndings(" ")}";
        }
    }
}
